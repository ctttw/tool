...
const twZipCodes: Record<string, Record<string, string>> = {
  ...
  "苗栗縣": {"竹南鎮": "350", "頭份市": "351", "三灣鄉": "352", "南庄鄉": "353", "獅潭鄉": "354", "後龍鎮": "356", "通霄鎮": "357", "苑裡鎮": "358", "苗栗市": "360", "造橋鄉": "361", "頭屋鄉": "362", "公館鄉": "363", "大湖鄉": "364", "泰安鄉": "365", "銅鑼鄉": "366", "三義鄉": "367", "西湖鄉": "368"},
  ...
  "彰化縣": {"彰化市": "500", "芬園鄉": "502", "花壇鄉": "503", "秀水鄉": "504", "鹿港鎮": "505", "福興鄉": "506", "線西鄉": "507", "和美鎮": "508", "伸港鄉": "509", "員林市": "510", "社頭鄉": "511", "田中鎮": "512", "二水鄉": "513", "溪湖鎮": "514", "大村鄉": "515", "埔鹽鄉": "516", "埔心鄉": "517", "埤頭鄉": "521", "田尾鄉": "522", "竹塘鄉": "523", "二林鎮": "524", "大城鄉": "525", "芳苑鄉": "526", "溪州鄉": "527", "北斗鎮": "528"},
  ...
  "臺東縣": {"臺東市": "930", "綠島鄉": "951", "蘭嶼鄉": "952", "延平鄉": "953", "卑南鄉": "954", "鹿野鄉": "955", "關山鎮": "956", "海端鄉": "957", "池上鄉": "958", "東河鄉": "959", "成功鎮": "961", "長濱鄉": "962", "太麻里鄉": "963", "金峰鄉": "964", "大武鄉": "965", "達仁鄉": "966"},
...
export default function ZipCode() {
  ...
  const cities = Object.keys(twZipCodes);
  const [selectedCity, setSelectedCity] = useState(cities[0]);
  const districts = Object.keys(twZipCodes[selectedCity] || {});
  const [selectedDistrict, setSelectedDistrict] = useState(districts[0]);

  const handleCityChange = (e: ChangeEvent<HTMLSelectElement>) => {
    const newCity = e.target.value;
    setSelectedCity(newCity);
    const newDistricts = Object.keys(twZipCodes[newCity] || {});
    setSelectedDistrict(newDistricts[0]);
  };

  const zipCode = twZipCodes[selectedCity]?.[selectedDistrict] || t('zipcode.noData');

  return (
    <>

      <Helmet>
        <title>{t('zipcode.title')} - Utility Lab</title>
        <meta name="description" content={t('zipcode.desc1', t('home.desc'))} />
      </Helmet>
      <div className="max-w-xl mx-auto space-y-12 animate-in fade-in zoom-in-95 duration-300 p-8 pt-12">
      <header className="text-center">
        <h1 className="text-4xl font-serif italic tracking-tight text-fg-editorial mb-4">{t('zipcode.title')}</h1>
        <p className="text-xs uppercase tracking-widest font-mono opacity-50 mb-6">{t('zipcode.subtitle')}</p>
        <p className="text-sm opacity-70 leading-relaxed font-medium">
          {t('zipcode.desc1')}<br/><br/>
          {t('zipcode.desc2')}
        </p>
      </header>
      ...
      <div className="border border-fg-editorial p-8 md:p-12 bg-transparent">
        <div className="space-y-8">
          <div>
            <label className="block text-xs uppercase tracking-widest font-mono opacity-60 mb-2 flex items-center">
              {t('zipcode.city')}
              <GuideModal title={t('zipcode.guideTitle')}>
                <ul className="list-disc pl-5 space-y-2">
                  <li><strong>{t('common.inputRule')}</strong>：{t('zipcode.guide1')}</li>
                  <li><strong>{t('common.calcLogic')}</strong>：{t('zipcode.guide2')}</li>
                </ul>
              </GuideModal>
            </label>
            <select
              ...
              className="w-full bg-transparent border-b border-fg-editorial text-fg-editorial text-lg rounded-none focus:border-b-2 block p-2 outline-none transition-all cursor-pointer font-serif"
            >
              {cities.map(city => (
          ...
          <div>
            <label className="block text-xs uppercase tracking-widest font-mono opacity-60 mb-2">{t('zipcode.district')}</label>
            <select
        ...
        <div className="mt-16 pt-12 border-t border-fg-editorial flex flex-col items-center justify-center text-center">
          <p className="text-[10px] font-mono tracking-[0.2em] uppercase opacity-50 mb-6">ZIP CODE</p>
          <div className="text-8xl font-serif italic tracking-tighter">
            {zipCode}
          </div>
          <div className="mt-6 text-lg font-mono opacity-70">
